...
public abstract class ChainConveyorBlockMixin {
    ...
    private boolean createVibrantVaults$useIsFrogport(boolean original, @Local(argsOnly = true) Player player, @Local(argsOnly = true) InteractionHand hand) {
        return original || ModItemTags.VIBRANT_FROGPORTS.matches(player.getItemInHand(hand));
    }
}
